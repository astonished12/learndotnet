...
namespace EventApp.Services
{
    public class TestService
    {
        ...
        public IEnumerable<GuestDTO> GetMajorGuests()
        {
            ...
            return guests.Select(g => new GuestDTO().InjectFrom(g) as GuestDTO).ToList();
        }
        ...
        {
            var staffs = Context.Staffs.Where(s => s.LocationId == null)
                                       .Select(s => new StaffDetailDTO { FirstName = s.FirstName, LastName = s.LastName, Email = s.Email, Phone = s.Phone })
                                       .ToList();

            return staffs.Select(s => new StaffDetailDTO().InjectFrom(s) as StaffDetailDTO).ToList();
        }
        ...
        public IEnumerable<GuestDetailDTO> GetGuestsDetailsFromExpertNetowrkParty()
        {
            ...
            return guests.Select(eg => new GuestDetailDTO { FirstName = eg.Guest.FirstName, LastName = eg.Guest.LastName, Email = eg.Guest.Email })
                         .ToList();
